...
class TriggerDispatch:
    def __init__(self, mcu):
        ...
        self._use_adaptive = (timeout_mode == 'adaptive')
    ...
    def add_stepper(self, stepper):
        ...
        if trsync is None:
            ...
            # 为新的 MCU 创建 adaptive timeout 实例
            if self._use_adaptive:
                mcu = stepper.get_mcu()
                printer = mcu.get_printer()
                config = printer.lookup_object('configfile').read_main_config()
                self._adaptive_timeout[mcu] = trsync_adaptive.TRSyncAdaptive(
                    config, mcu)
    ...
    def start(self, print_time):
        ...
        # 计算 expire_timeout
        if self._use_adaptive:
            # Adaptive 模式：为每个 trsync 更新 RTT 并计算 timeout
            for trsync in self._trsyncs:
                mcu = trsync.get_mcu()
                if mcu not in self._adaptive_timeout:
                    # 首次使用该 MCU，创建 adaptive 实例
                    printer = mcu.get_printer()
                    config = printer.lookup_object('configfile').read_main_config()
                    self._adaptive_timeout[mcu] = trsync_adaptive.TRSyncAdaptive(
                        config, mcu)
                ...
                # 更新 RTT 统计
                self._adaptive_timeout[mcu].update()

            # 使用所有 MCU 中最大的 timeout
            expire_timeout = max(
                self._adaptive_timeout[trsync.get_mcu()].get_timeout()
                for trsync in self._trsyncs
            )

            logging.info("TriggerDispatch.start: Using adaptive timeout=%.6f s "
                        "for %d MCU(s)", expire_timeout, len(self._trsyncs))
        else:
            # Fixed 模式：使用原始逻辑
